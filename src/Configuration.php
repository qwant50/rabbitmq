...
class Configuration extends Component
{
     ...
     * @throws \yii\di\NotInstantiableException
     */
    public function getProducer(string $producerName) : ProducerInterface
    {
    ...
    protected function validateRequired()
    {
        ...
        foreach ($this->producers as $producer) {
            ...
            $allowed = ['rpc', 'event'];
            if (!in_array($producer['type'], $allowed, true)) {
                $allowed = implode(', ', $allowed);
                throw new InvalidConfigException("Unknown producer type `{$producer['type']}`. Allowed values are: {$allowed}");
            }
        }
        foreach ($this->consumers as $consumer) {
            ...
            $allowed = ['rpc', 'event'];
            if (!in_array($consumer['type'], $allowed, true)) {
                $allowed = implode(', ', $allowed);
                throw new InvalidConfigException("Unknown consumer type `{$consumer['type']}`. Allowed values are: {$allowed}");
            }
        }
    }
